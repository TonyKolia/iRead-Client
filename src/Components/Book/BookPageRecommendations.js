            ...
            .then(response => {
                if (response.success)
                    ...
                    if (response.statusCode == 404)
                        setRecommendedBooks({ recommendedForUser: [], recommendedByOthers: [], similar: [] });
                    else
                        navigate("/error");
                }
            });
            ...
            <div id="recommended" className="collapse show">
                ...
                </div>
                {
                    selectedTab == 1 && <div style={{marginTop: "10px"}} className="row row-cols-1 row-cols-md-4 card-custom-container book-page-recommendations">
                        {recommendedBooks.recommendedForUser?.length === 0 ? <div className="not-found-container"><h3>Δεν βρέθηκαν βιβλία</h3></div> : recommendedBooks.recommendedForUser?.map(book => <BookItem key={book.id} book={book} />)}
                    </div>
                }
                {
                ...
                }
                {
                    selectedTab == 3 && <div style={{marginTop: "10px"}} className="row row-cols-1 row-cols-md-4 card-custom-container book-page-recommendations">
                        {recommendedBooks.similar?.length === 0 ? <div style={{ width: "100%" }}><h5 style={{ textAlign: "center" }}>Δεν βρέθηκαν βιβλία</h5></div> : recommendedBooks.similar?.map(book => <BookItem key={book.id} book={book} />)}
                    </div>
                }
            </div>
            <hr />
